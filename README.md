...
1. 레포지토리 클론
   ```bash
   git clone https://github.com/your-organization/CoreService.git
   cd CoreService
   ```
...
2. MySQL 데이터베이스 설정
   ...
   ```
   ```sql
   CREATE DATABASE coreservice;
   CREATE USER 'coreservice'@'localhost' IDENTIFIED BY 'your_password';
   GRANT ALL PRIVILEGES ON coreservice.* TO 'coreservice'@'localhost';
   FLUSH PRIVILEGES;
   ```
...
3. `application.properties` 또는 `application.yml` 파일 설정
   ```properties
   # 데이터베이스 설정
   spring.datasource.url=jdbc:mysql://localhost:3306/coreservice?useSSL=false&serverTimezone=UTC
   spring.datasource.username=coreservice
